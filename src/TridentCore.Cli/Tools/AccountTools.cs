...
public class AccountTools(AccountStore accounts)
{
    ...
    public string List() => JsonSerializer.Serialize(AccountOperation.List(accounts), McpJson.Options);

    [McpServerTool(Name = "account_add_offline")]
    [Description("Add an offline account.")]
    ...
    [McpServerTool(Name = "account_remove")]
    [Description("Remove a registered account by UUID.")]
    public string Remove([Description("Account UUID to remove")] string uuid) =>
