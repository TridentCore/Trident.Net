...
{
    [McpServerTool(Name = "trident_account_list"), Description("List all registered accounts.")]
    public string List()
        => JsonSerializer.Serialize(AccountOperation.List(accounts), McpJson.Options);

    [McpServerTool(Name = "trident_account_add_offline"), Description("Add an offline account.")]
    ...
    [McpServerTool(Name = "trident_account_remove"), Description("Remove a registered account by UUID.")]
