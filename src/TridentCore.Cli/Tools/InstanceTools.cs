...
{
    [McpServerTool(Name = "trident_instance_list"), Description("List all Trident instances.")]
    public string List()
        => JsonSerializer.Serialize(InstanceOperation.List(profileManager), McpJson.Options);

    [McpServerTool(Name = "trident_instance_inspect"), Description("Inspect a Trident instance with package preview.")]
    ...
    [McpServerTool(Name = "trident_instance_create"), Description("Create a new Trident instance.")]
    ...
    [McpServerTool(Name = "trident_instance_unlock"), Description("Unlock an instance from its import source.")]
    public string Unlock(
        [Description("Instance key")] string instance,
        [Description("Profile file path (optional)")] string? profile = null)
        => JsonSerializer.Serialize(InstanceOperation.Unlock(resolver, profileManager, instance, profile), McpJson.Options);

    [McpServerTool(Name = "trident_instance_delete"), Description("Delete a Trident instance.")]
    public string Delete(
        [Description("Instance key")] string instance,
        [Description("Profile file path (optional)")] string? profile = null)
        => JsonSerializer.Serialize(InstanceOperation.Delete(resolver, profileManager, instance, profile), McpJson.Options);

    [McpServerTool(Name = "trident_instance_reset"), Description("Reset build artifacts for a Trident instance.")]
    public string Reset(
        [Description("Instance key")] string instance,
        [Description("Profile file path (optional)")] string? profile = null)
        => JsonSerializer.Serialize(InstanceOperation.Reset(resolver, instanceManager, instance, profile), McpJson.Options);

    [McpServerTool(Name = "trident_instance_build"), Description("Build a Trident instance.")]
    public async Task<string> Build(
        [Description("Instance key")] string instance,
        [Description("Fast mode (optional)")] bool fastMode = false,
        [Description("Resolve dependency (optional)")] bool resolveDependency = false,
        [Description("Full check (optional)")] bool fullCheck = false,
        [Description("Java home path (optional)")] string? javaHome = null,
        [Description("Profile file path (optional)")] string? profile = null)
        => JsonSerializer.Serialize(await InstanceOperation.BuildAsync(resolver, instanceManager, instance, profile, fastMode, resolveDependency, fullCheck, javaHome), McpJson.Options);

    [McpServerTool(Name = "trident_instance_export"), Description("Export a Trident instance to a pack file.")]
    ...
    [McpServerTool(Name = "trident_instance_import"), Description("Import a pack file as a new Trident instance.")]
