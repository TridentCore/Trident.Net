    ...
    ExporterAgent exporterAgent)
{
    ...
    [Description("List all Trident instances.")]
    public string List() => JsonSerializer.Serialize(InstanceOperation.List(profileManager), McpJson.Options);

    [McpServerTool(Name = "instance_inspect")]
    [Description("Inspect a Trident instance with package preview.")]
    public async Task<string> Inspect(
        ...
        JsonSerializer.Serialize(await InstanceOperation.Inspect(resolver, repositories, instance, profile),
                                 McpJson.Options);

    [McpServerTool(Name = "instance_create")]
    [Description("Create a new Trident instance.")]
    public string Create(
        ...
        JsonSerializer.Serialize(InstanceOperation.Create(profileManager, name, version, loader, identity),
                                 McpJson.Options);

    [McpServerTool(Name = "instance_unlock")]
    [Description("Unlock an instance from its import source.")]
    public string Unlock(
        [Description("Instance key")] string instance,
        [Description("Profile file path (optional)")] string? profile = null) =>
        JsonSerializer.Serialize(InstanceOperation.Unlock(resolver, profileManager, instance, profile),
                                 McpJson.Options);

    [McpServerTool(Name = "instance_delete")]
    [Description("Delete a Trident instance.")]
    public string Delete(
        [Description("Instance key")] string instance,
        [Description("Profile file path (optional)")] string? profile = null) =>
        JsonSerializer.Serialize(InstanceOperation.Delete(resolver, profileManager, instance, profile),
                                 McpJson.Options);

    [McpServerTool(Name = "instance_reset")]
    [Description("Reset build artifacts for a Trident instance.")]
    public string Reset(
        [Description("Instance key")] string instance,
        [Description("Profile file path (optional)")] string? profile = null) =>
        JsonSerializer.Serialize(InstanceOperation.Reset(resolver, instanceManager, instance, profile),
                                 McpJson.Options);

    [McpServerTool(Name = "instance_export")]
    [Description("Export a Trident instance to a pack file.")]
    public async Task<string> Export(
        ...
        [Description("Profile file path (optional)")] string? profile = null) =>
        JsonSerializer.Serialize(await InstanceOperation.ExportAsync(resolver,
                                                                     exporterAgent,
                                                                     instance,
                                                                     profile,
                                                                     format,
    ...
    [McpServerTool(Name = "instance_import")]
    [Description("Import a pack file as a new Trident instance.")]
    ...
    [McpServerTool(Name = "instance_install")]
    [Description("Install a modpack from a repository as a new Trident instance.")]
