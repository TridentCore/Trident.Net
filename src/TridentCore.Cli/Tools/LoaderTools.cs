...
{
    [McpServerTool(Name = "loader_list")]
    [Description("List all supported loaders.")]
    public string List() => JsonSerializer.Serialize(LoaderOperation.List(), McpJson.Options);

    [McpServerTool(Name = "loader_version_list")]
    [Description("List available versions of a loader for a Minecraft version.")]
    public async Task<string> VersionList(
        ...
        JsonSerializer.Serialize(await LoaderOperation.VersionList(prismLauncher,
                                                                   loaderId,
                                                                   version,
                                                                   sort,
                                                                   index,
    ...
    [McpServerTool(Name = "loader_get")]
    [Description("Get the loader configuration for an instance.")]
    public string Get(
    ...
    [McpServerTool(Name = "loader_set")]
    [Description("Set the loader for an instance.")]
    public string Set(
        [Description("Loader LURL, e.g. fabric:0.16.0")] string loader,
        [Description("Instance key")] string instance,
        [Description("Profile file path (optional)")] string? profile = null) =>
        JsonSerializer.Serialize(LoaderOperation.Set(resolver, profileManager, loader, instance, profile),
                                 McpJson.Options);
}
