...
{
    [McpServerTool(Name = "repository_list")]
    [Description("List all configured repositories.")]
    public string List() =>
        JsonSerializer.Serialize(RepositoryOperation.List(userRepositories, combined), McpJson.Options);

    [McpServerTool(Name = "repository_status")]
    [Description("Check repository status and capabilities.")]
    public async Task<string> Status([Description("Repository label (optional)")] string? label = null) =>
        JsonSerializer.Serialize(await RepositoryOperation.Status(repositories, label), McpJson.Options);

    [McpServerTool(Name = "repository_add")]
    [Description("Add or replace a user-configured repository.")]
    public string Add(
        ...
        JsonSerializer.Serialize(RepositoryOperation.Add(userRepositories, label, driver, endpoint, apiKey, userAgent),
                                 McpJson.Options);

    [McpServerTool(Name = "repository_remove")]
    [Description("Remove a user-configured repository.")]
    public string Remove([Description("Repository label")] string label) =>
