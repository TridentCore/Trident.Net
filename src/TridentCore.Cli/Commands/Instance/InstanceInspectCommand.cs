...
public class InstanceInspectCommand(InstanceContextResolver resolver, RepositoryAgent repositories, CliOutput output)
    : InstanceCommandBase<InstanceInspectCommand.Arguments>(resolver)
{
    ...
    private async Task InspectAsync(Arguments settings, CancellationToken cancellationToken)
    {
        ...
        output.WriteKeyValueTable("Instance details",
                                  ("Key", dto.Key),
                                  ("Name", dto.Name),
                                  ("Version", dto.Version),
                                  ("Loader", dto.Loader),
        ...
        {
            output.WriteEmptyState("No packages", "Add packages with: trident package add --instance <key> <pref>");
            return;
        }

        output.WriteTable(PackageCliHelper.CreatePackageTable("Package preview", dto.Packages));
        if (dto.HiddenPackageCount > 0)
        {
            output.WriteInfo($"Showing {dto.Packages.Count} of {dto.PackageCount} packages. Use 'trident package list --instance {dto.Key}' for the full list.");
        }
    }
