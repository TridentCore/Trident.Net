    ...
    CliOutput output) : InstanceCommandBase<InstanceRunCommand.Arguments>(resolver)
{
    ...
    private async Task RunAsync(Arguments settings, CancellationToken cancellationToken)
    {
        ...
        var forcedJava = settings.JavaHome ?? profile.GetOverride<string>(TridentProfile.OVERRIDE_JAVA_HOME);
        var locator = !string.IsNullOrWhiteSpace(forcedJava)
            ? JavaHelper.MakeForcedLocator(forcedJava)
            : JavaHelper.MakeLocator(_ => configuration.Get<string>(TridentProfile.OVERRIDE_JAVA_HOME));
