...
public class PackageInspectCommand(InstanceContextResolver resolver, RepositoryAgent repositories, CliOutput output)
    : Command<PackageInspectCommand.Arguments>
{
    ...
    private async Task InspectAsync(Arguments settings)
    {
        ...
        output.WriteKeyValueTable("Package details",
        ...
        {
            AnsiConsole.Write(new Panel(Markup.Escape(result.Package.Summary)).Header("Summary").RoundedBorder());
        }
        ...
        output.WriteTable(PackageCliHelper.CreateDependencyTable("Dependencies", result.Package.Dependencies));
    }
