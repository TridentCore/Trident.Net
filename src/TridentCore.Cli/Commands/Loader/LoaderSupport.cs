...
internal static class LoaderSupport
{
    ...
    [
        .. PrismLauncherService.UidMappings.Select(x => new LoaderInfo(x.Key,
                                                                       LoaderHelper.ToDisplayName(x.Key),
                                                                       x.Value))
    ];

    public static bool IsSupported(string identity) => PrismLauncherService.UidMappings.ContainsKey(identity);
