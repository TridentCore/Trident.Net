...
public class CliOutput(CliContext context)
{
    ...
    public void WriteData<T>(T value) => Console.Out.WriteLine(JsonSerializer.Serialize(value, JSON_OPTIONS));
    ...
    public void WriteError(string message)
    {
        ...
        AnsiConsole.Write(new Panel($"[bold red]{Markup.Escape(message)}[/]")
    ...
    public void WriteEmptyState(string title, string hint) =>
        AnsiConsole.Write(new Panel($"[bold]{Markup.Escape(title)}[/]\n[dim]{Markup.Escape(hint)}[/]")
                         .RoundedBorder()
    ...
    public void RequireConfirmation(string prompt, bool confirmed)
    {
        ...
        if (!IsInteractive)
        {
            throw new CliException("Confirmation is required. Re-run with --yes in non-interactive mode.",
                                   ExitCodes.CANCELED);
        }
    ...
    public static string FormatStatus(string value, string color) => $"[{color}]{Markup.Escape(value)}[/]";
}
