...
{
    // 隐藏仓库仍注册、可按 label 解析，但从 RepositoryAgent.Labels 排除，
    // 永不现身浏览/搜索/市场列表。
    bool IsHidden => false;
    ...
    // WARNING: batch counterpart of IdentifyAsync. The repository queries its native batch endpoint with
    ...
    // WARNING: batch counterpart of RecognizeAsync. The result is total — every input uri lands in
