...
public sealed record BatchResolveResult<TIdentifier, TItem>(
    ...
    IReadOnlyDictionary<TIdentifier, Exception> Failed
) where TIdentifier : notnull
{
    ...
    public BatchResolveResult<TMappedIdentifier, TItem> MapKeys<TMappedIdentifier>(
        Func<TIdentifier, TMappedIdentifier> map
    ) where TMappedIdentifier : notnull => new(
        Successful.ToDictionary(x => map(x.Key), x => x.Value),
        Failed.ToDictionary(x => map(x.Key), x => x.Value)
    );
    ...
    public static BatchResolveResult<TIdentifier, TItem> FromFailures(
        ...
        new Dictionary<TIdentifier, TItem>(),
        identifiers.Distinct().ToDictionary(x => x, _ => error)
    );
}
