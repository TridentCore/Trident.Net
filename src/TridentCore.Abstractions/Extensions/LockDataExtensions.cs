...
{
    // WARNING: FastMode 门——仅当 platform、deploy-options 指纹、叠加优先级指纹与声明（启用）包集合
        ...
        string? launchPlanHash)
    {
        ...
        if (self.Viability.LaunchPlanHash != launchPlanHash)
        {
            return false;
        }

        if (self.LaunchPlan is null)
        {
            return false;
        }
