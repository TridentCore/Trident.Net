...
public sealed class DeploymentIndexService(MojangService mojang, IHttpClientFactory factory)
{
    ...
    public async Task<LockData.RuntimeIndexReference?> ResolveRuntimeReferenceAsync(
        uint major, LockData.RuntimeIndexReference? reference, CancellationToken token)
    {
        if (await DeploymentIndexHelper.ReadRuntimeAsync(major, reference?.Hash, token).ConfigureAwait(false) is not null)
            return reference;
        return await FindRuntimeReferenceAsync(major).ConfigureAwait(false);
    }

    public async Task<RuntimeIndex> EnsureRuntimeAsync(uint major, LockData.RuntimeIndexReference? reference, CancellationToken token)
    {
        if (await DeploymentIndexHelper.ReadRuntimeAsync(major, reference?.Hash, token).ConfigureAwait(false) is { } cached) return cached;
        reference ??= await FindRuntimeReferenceAsync(major).ConfigureAwait(false);
        using var client = factory.CreateClient(RepositoryAgent.CLIENT_NAME);
        var path = PathDef.Default.FileOfRuntimeManifest(major);
        await DownloadHelper.DownloadAsync(client, reference.Url, path, reference.Hash, token).ConfigureAwait(false);
        return await DeploymentIndexHelper.ReadRuntimeAsync(major, reference.Hash, token).ConfigureAwait(false)
            ?? throw new InvalidDataException("Downloaded runtime index is invalid.");
    }

    private async Task<LockData.RuntimeIndexReference> FindRuntimeReferenceAsync(uint major)
    {
        ...
        return new(selected.Manifest.Url, FileHash.Sha1(selected.Manifest.Sha1));
    }
}
