...
public class XboxLiveService(IXboxLiveClient liveClient, IXboxServiceClient serviceClient)
{
    ...
    public async Task<XboxLiveResponse> AuthenticateForXboxLiveTokenByMicrosoftTokenAsync(
        string microsoftToken
    ) =>
        EnsureResponseStatus(
            await liveClient
                .AcquireXboxLiveTokenAsync(
                    new(new($"d={microsoftToken}"), "http://auth.xboxlive.com")
                )
                .ConfigureAwait(false)
        );

    public async Task<XboxLiveResponse> AuthorizeForServiceTokenByXboxLiveTokenAsync(
        string xboxLiveToken
    ) =>
        EnsureResponseStatus(
            await serviceClient
                .AcquireMinecraftTokenAsync(
                    new(new([xboxLiveToken]), "rp://api.minecraftservices.com/")
                )
                .ConfigureAwait(false)
        );
    ...
    private static XboxLiveResponse EnsureResponseStatus(XboxLiveResponse response)
    {
        if (response.XErr.HasValue)
        {
            var kind = response.XErr.Value switch
            {
                ...
                _ => XboxLiveAuthenticationException.ErrorKind.Unkown,
            };
