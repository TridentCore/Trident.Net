...
public class AtLauncherAdapter(ILogger<AtLauncherAdapter>? logger = null) : ILauncherAdapter
{
    ...
    // loaderVersion.type（不区分大小写）→ loader 标识。LegacyFabric 映射 Fabric；Paper/Purpur
    ...
    public string? DefaultDataDirectory(LauncherKind kind)
    {
        ...
        // ATLauncher 默认便携——数据目录在可执行文件旁，无固定系统位。
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (data is not null)
        {
            // 顶层 `id`（继承自 Mojang 的 MinecraftVersion）即 Minecraft 版本。
            version = data.Id;
        ...
        // ATLauncher 直接部署进实例文件夹——无嵌套 .minecraft 层。
        return new()
        {
