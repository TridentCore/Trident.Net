...
public class AuthlibAccountConfigurer : IAccountConfigurer
{
    ...
    public async Task ConfigureLaunchAsync(IAccount account, AccountConfigurerAgent.LaunchContext context, CancellationToken token)
    {
        ...
        if (aiLib is null)
            throw new AccountConfigurationException(
                $"Authlib-injector library not found in artifact for account {ai.Username}. " +
    ...
    {
        var ai = (AuthlibAccount)account;
        return await _yggdrasil.ValidateAsync(ai.ServerUrl, ai.AccessToken, ai.ClientToken, token).ConfigureAwait(false);
    }
    ...
    public async Task RefreshAsync(IAccount account, CancellationToken token)
    {
        ...
        try
        {
            var response = await _yggdrasil.RefreshAsync(
                ai.ServerUrl,
                ai.AccessToken,
                ai.ClientToken!,
        ...
        catch (ApiException ex)
        {
            throw new AccountAuthenticationException(
                "Unable to refresh the expired authlib-injector session. Please re-authenticate.", ex);
        }
    }
}
