using Refit;
using TridentCore.Core.Models.MicrosoftApi;
...
public interface IMicrosoftClient
{
    ...
    Task<TokenResponse> AuthenticateAsync(
        [Body(BodySerializationMethod.UrlEncoded)] AuthenticateRequest request
    );

    [Post("/consumers/oauth2/v2.0/token")]
    Task<TokenResponse> RefreshUserAsync(
        [Body(BodySerializationMethod.UrlEncoded)] RefreshUserRequest request
    );
}
